...
namespace FixMath.NET
{
    public class Fix64Tests
    {
        ...
        //}
        private static object[] SinTestCases()
        {
            return new[]
            {
                new [] { Fix64.Zero, Fix64.Zero },
                new [] { Fix64.PiOver2, Fix64.One },
                new [] { (Fix64.Pi), Fix64.Zero },
                new [] { (Fix64.Pi + Fix64.PiOver2), -Fix64.One },
                new [] { (Fix64.PiTimes2), Fix64.Zero },
                new [] { (-Fix64.PiOver2), -Fix64.One },
                new [] { (-Fix64.Pi), Fix64.Zero },
                new [] { (-Fix64.Pi - Fix64.PiOver2), Fix64.One },
                new [] { (-Fix64.PiTimes2), Fix64.Zero },
        ...
        [TestCaseSource(nameof(SinTestCases))]
        public void Sin(Fix64 x, Fix64 expectedResult)
        {
            var result = Fix64.Sin(x);
        ...
        public void TestSinPrecision()
        {
            ...
            {
                var f = (Fix64)angle;
                var actualF = CalculateDelta(f, out var expected, out var delta);
                Assert.True(delta <= 3 * Fix64.Precision,
                    string.Format("Sin({0}): expected {1} but got {2}", angle, expected, actualF));
            }
            ...
            foreach (var val in m_testCases)
            {
                ...
                var actualF = CalculateDelta(f, out var expected, out var delta);
                Assert.True(delta <= 0.0000001M,
                    string.Format("Sin({0}): expected {1} but got {2}", f, expected, actualF));
            }
        }

        private static Fix64 CalculateDelta(Fix64 f, out decimal expected, out decimal delta)
        {
            var actualF = Fix64.Sin(f);
            expected = (decimal)Math.Sin((double)f);
            delta = Math.Abs(expected - (decimal)actualF);
            return actualF;
        }
